...
public class boatScriptQueue : MonoBehaviour {
    ...
    [Tooltip("Total number of queue elements to be stored.")]
    public int maxQueueLength = 10;
    Queue<Vector3> oarReadings = new Queue<Vector3>();
    ...
    void Start() {
        ...
        for (int i = 0; i < maxQueueLength; i++) {
            oarReadings.Enqueue(Vector3.zero);
        }
    }
    ...
    Vector3 getAverageVector() {

        oarReadings.Dequeue();
        oarReadings.Enqueue(lastOarPosition - nowOarPosition);

        Debug.Assert(oarReadings.Count == maxQueueLength);
        float vec_x = 0;
        float vec_y = 0;
        float vec_z = 0;
        foreach (Vector3 reading in oarReadings) {
            vec_x += reading.x;
            vec_y += reading.y;
            vec_z += reading.z;
        }
    ...
    void Update() {
        ...
        Vector3 deltaOarPosition = getAverageVector();
